...
table(raw_data$cps19_province, useNA = "always")
clean_data$subgroup <- NA
clean_data$subgroup[raw_data$cps19_province == "Quebec"] <- "qc"
clean_data$subgroup[raw_data$cps19_province != "Quebec"] <- "can"
...
table(raw_data$cps19_rel_imp)
clean_data$importance <- NA
clean_data$importance[raw_data$cps19_rel_imp == "Very important"] <- 1
clean_data$importance[raw_data$cps19_rel_imp == "Somewhat important"] <- 0.67
clean_data$importance[raw_data$cps19_rel_imp == "Not very important"] <- 0.33
clean_data$importance[raw_data$cps19_rel_imp == "Not important at all"] <- 0
